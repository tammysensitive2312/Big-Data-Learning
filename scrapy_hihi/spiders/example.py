...
class AmazonSearchSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for keyword in keyword_list:
            ...
            yield scrapy.Request(url=amazon_search_url, callback=self.discover_products_urls, meta={'keyword': keyword, 'page': 1})

    def discover_products_urls(self, response):
        ...
        for product in search_products:
            ...
            if page == 1:
                ...
                for page_num in range(2, int(last_page)):
                    ...
                    yield scrapy.Request(url=amazon_search_url, callback=self.discover_products_urls, meta={'keyword': keyword, 'page': page_num})

    def parse_product_data(self, response):
        image = json.loads(re.findall(r"colorImages':.*'initial':\s*(\[.+?\])},\n", response.text)[0])
        variant = re.findall(r'dimensionValuesDisplayData"\s*:\s* ({.+?}),\n', response.text)
